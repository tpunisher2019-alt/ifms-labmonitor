export type Device = { id:string; hostname:string; agent_version:string; os_type:string; status:string; last_seen_at:string; inventory_collected_at:string|null; software_count?:number };
...
export type Release = { id:string; version:string; platform:"windows"|"linux"; active:boolean; created_at:string };
...
const demo: DashboardData = {
  configured:false,
  devices:[
    {id:"1",hostname:"LAB01-PC01",agent_version:"2.0.0",os_type:"Windows",status:"online",last_seen_at:new Date().toISOString(),inventory_collected_at:new Date().toISOString(),software_count:47},
    {id:"2",hostname:"LAB01-PC07",agent_version:"2.0.0",os_type:"Windows",status:"online",last_seen_at:new Date(Date.now()-120000).toISOString(),inventory_collected_at:new Date().toISOString(),software_count:44},
    {id:"3",hostname:"LAB02-PC12",agent_version:"1.0.0",os_type:"Debian",status:"offline",last_seen_at:new Date(Date.now()-86400000).toISOString(),inventory_collected_at:null,software_count:39},
  ],
  events:[
  ...
  releases:[{id:"r1",version:"2.0.0",platform:"windows",active:true,created_at:new Date().toISOString()}], softwareTotal:130,
  software:[
...
export async function getDashboardData():Promise<DashboardData>{
  try{
    const [devices,events,releases,inventory,jobs,settings]=await Promise.all([
      rest("devices?select=id,hostname,agent_version,os_type,status,last_seen_at,inventory_collected_at&order=hostname"),
      ...
      rest("agent_releases?select=id,version,platform,active,created_at&order=created_at.desc"),
