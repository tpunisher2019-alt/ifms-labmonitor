...
const state = { profile: null, devices: [], events: [], software: [], releases: [], jobs: [], requests: [], storageMetrics: null, selected: new Set(), selectedRequests: new Set(), retentionDays: 90, remoteUpdatesEnabled: true, autoAuthorizeKnownDevices: true, updatePlatform: "windows" };
...
const platformFromOs = (value) => { const os = String(value || "").trim().toLowerCase(); if (os.startsWith("win")) return "windows"; if (["linux", "debian", "ubuntu"].includes(os)) return "linux"; return os; };
...
function renderJobHistory() {
  ...
  $("update-job-rows").innerHTML = updateJobs.map((j) => `<tr><td>${esc(j.devices?.hostname)}</td><td>${esc(j.devices?.os_type || "—")}</td><td><span class="badge">${esc(j.status)}</span></td><td>${j.leased_at ? new Date(j.leased_at).toLocaleString("pt-BR") : "Aguardando"}</td><td>${j.completed_at ? new Date(j.completed_at).toLocaleString("pt-BR") : "—"}</td><td>${esc(j.result?.message || "—")}</td></tr>`).join("") || emptyRow(6, "Nenhuma atualização enviada.");
}
...
function renderRequests() {
  ...
  $("known-device-status").textContent = state.autoAuthorizeKnownDevices ? "Máquinas conhecidas: liberação automática" : "Máquinas conhecidas: aprovação manual";
  $("known-device-status").className = `badge ${state.autoAuthorizeKnownDevices ? "active" : "inactive"}`;
  $("toggle-known-device-auto-approval").textContent = state.autoAuthorizeKnownDevices ? "Desabilitar automação" : "Habilitar automação";
...
async function createJob(type, releaseId) {
  const deviceIds = type === "agent_update" ? compatibleSelectedIds() : [...state.selected];
  if (!deviceIds.length) return;
  const { error } = await supabase.rpc("create_device_job", { job_type: type, device_ids: deviceIds, release_id: releaseId || null });
  if (error) return message(type === "agent_update" && !state.remoteUpdatesEnabled ? "Habilite as atualizações remotas antes de enviar." : type === "agent_update" ? "A versão e os computadores precisam pertencer ao mesmo sistema operacional." : "Não foi possível criar a tarefa.", true);
...
$("software-search").addEventListener("input", renderSoftware); $("software-device").addEventListener("change", renderSoftware); $("release-select").addEventListener("change", updateActionButtons);
$("update-platform-select").addEventListener("change", (event) => { state.updatePlatform = event.target.value; state.selected.clear(); render(); });
$("inventory-refresh").addEventListener("click", () => createJob("inventory_refresh")); $("download-release").addEventListener("click", downloadRelease); $("send-update").addEventListener("click", () => createJob("agent_update", $("release-select").value));
...
$("toggle-known-device-auto-approval").addEventListener("click", async () => { const enabled = !state.autoAuthorizeKnownDevices; const { error } = await supabase.from("system_settings").update({ auto_authorize_known_devices: enabled, updated_at: new Date().toISOString(), updated_by: state.profile.email }).eq("id", true); if (error) return message("Não foi possível alterar a autorização automática.", true); state.autoAuthorizeKnownDevices = enabled; message(enabled ? "Máquinas conhecidas serão liberadas automaticamente." : "Todas as instalações passarão por aprovação manual."); renderRequests(); });
